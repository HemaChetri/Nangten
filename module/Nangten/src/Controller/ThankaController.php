...
class ThankaController  extends AbstractActionController
{
	private $_container;
	...
	{  
		...
		$id = $this->_id;
			...
			$lhakhangsectionTable = $this->getDefinedTable(Nangten\NangtenTable::class)->getReport($data,array('TenGroupId'=>11));
        ...
        return new ViewModel(array(
			...
			'category'			=>$this->getDefinedTable(Nangten\CategoryTable::class)->get(array('group'=>11)),
	 ...
	 * Add thanka  Action
	 */
	public function addthankaAction()
	{
		$this->init();
		$page = $this->_id;
		if($this->getRequest()->isPost()){
			...
			$data = [
				...
				'TenTypeDz'      		=>$form['TenTypeDz'],
				'TenSideMatDz'      	=>$form['TenSideMatDz'],
				'TenBotmMatDz'    		=>$form['TenBotmMatDz'],
				'TenCoverDz'      		=>$form['TenCoverDz'],
				'TenStickBotmDz'      	=>$form['TenStickBotmDz'],
				'TenHolderDz'      		=>$form['TenHolderDz'],
				'TenBuilderDz'      	=>$form['TenBuilderDz'],
				'TenBackingDz'      	=>$form['TenBackingDz'],
				'TenSourceDz'      		=>$form['TenSourceDz'],
				'TenAreaBoarderDz'     	=>$form['TenAreaBoarderDz'],
				'TenMeasurePaintedArDz' =>$form['TenMeasurePaintedArDz'],
			...
			if($result){
					...
					$this->flashMessenger()->addMessage("success^ Successfully added new thanka details.");
					return $this->redirect()->toRoute('thanka', array('action' =>'thanka'));
				}else {
					$this->_connection->rollback(); 
					$this->flashMessenger()->addMessage("error^ Failed to add new thanka details.");
					return $this->redirect()->toRoute('thanka');
				}	
		}		
		return new ViewModel(array(
				'title'        => 'Add Thanka',
				...
				'material'	   =>$this->getDefinedTable(Nangten\MaterialTable::class)->get(array('GroupId'=>11)),
				'category'	   =>$this->getDefinedTable(Nangten\CategoryTable::class)->get(array('group'=>11)),
				'subcategory'  =>$this->getDefinedTable(Nangten\SubCategoryTable::class)->get(array('group'=>11)),
		));
	 ...
	 * Edit Thanka  Action
	 */
	public function editthankaAction()
	{
		$this->init();
		$page = $this->_id;
		if($this->getRequest()->isPost()){
			...
			if($result){
					...
					return $this->redirect()->toRoute('thanka', array('action' =>'viewthanka','id'=>$form['thanka_id']));
				}else {
					$this->_connection->rollback(); 
					$this->flashMessenger()->addMessage("error^ Failed to edit thanka details.");
					return $this->redirect()->toRoute('thanka');
				}	
		}		
		return new ViewModel(array(
				...
				'category'	=>$this->getDefinedTable(Nangten\CategoryTable::class)->get(array('group'=>11)),
				'subcategory'=>$this->getDefinedTable(Nangten\SubCategoryTable::class)->get(array('group'=>11)),
				'thanka'	=>$this->getDefinedTable(Nangten\NangtenTable::class)->get(['id'=>$this->_id]),
		));
	 ...
	 *upload Thanka image
	 */	
	public function addthankaimageAction()
	{	
		$this->init();
		...
		if ($request->isPost()) {
			...
			$thanka	=$this->getDefinedTable(Nangten\NangtenTable::class)->get($this->_id);
			foreach($thanka as $thankas);
			$dzongkhag	=$this->getDefinedTable(Administration\DistrictTable::class)->getColumn($thankas['DzongkhagId'],'DzongkhagNameEn');
			$gewog	=$this->getDefinedTable(Administration\BlockTable::class)->getColumn($thankas['GewogId'],'GewogNameEn');
			$lhakhang	=$this->getDefinedTable(Administration\LhakhangTable::class)->getColumn($thankas['LhakhangId'],'LhakhangNameEn');
			$folder_name =$this->_dir . '/thanka/' . $dzongkhag . '/' .$gewog. '/' .$lhakhang;
	...
	/*
	 *  Delete thanka  action
	 */
	public function deletethankaAction()
	{
		$this->init();
		$id = $this->_id;
		$array_id = explode("_", $id);
		$thanka_id = $array_id[0];
		if($this->getRequest()->isPost()){
			$form = $this->getRequest()->getpost();	
			$thankadtl = [				
				'id'      => $form['thanka_id'],
		  ...
		  if($thankadtl){
				...
				$this->_connection->commit();
				$this->flashMessenger()->addMessage("success^ Successfully removed the thanka.");
			}else {
				$this->_connection->rollback();
				$this->flashMessenger()->addMessage("error^ Failed to remove the thanka.");
			}
			return $this->redirect()->toRoute('thanka', array('action' =>'thanka'));
		}		
		$ViewModel = new ViewModel([
			'title'      => 'Remove thanka',
			'thanka'    => $this->getDefinedTable(Nangten\NangtenTable::class)->get($thanka_id),
		]);		
		$ViewModel->setTerminal(True);
		return $ViewModel;
	}
	 ...
	 *  View thanka action
	 */
	public function viewthankaAction()
	{  
    	$this->init(); 
		 return new ViewModel(array(
			'title'     		=> 'View Thanka',
