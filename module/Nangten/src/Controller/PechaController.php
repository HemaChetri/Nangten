...
class PechaController  extends AbstractActionController
{
	...
	{	
		...
		if ($request->isPost()) {
			...
			$dzongkhag	=$this->getDefinedTable(Administration\DistrictTable::class)->getColumn($pechas['DzongkhagId'],'DzongkhagNameEn');
			$gewog	=$this->getDefinedTable(Administration\BlockTable::class)->getColumn($pechas['GewogId'],'GewogNameEn');
			$lhakhang	=$this->getDefinedTable(Administration\LhakhangTable::class)->getColumn($pechas['LhakhangId'],'LhakhangNameEn');
			$folder_name =$this->_dir . '/pecha/' . $dzongkhag . '/' .$gewog. '/' .$lhakhang;
