...
class DebriController  extends AbstractActionController
{
	...
	public function editdebriAction()
	{
		...
		return new ViewModel(array(
				...
				'village'	 =>$this->getDefinedTable(Administration\VillageTable::class)->getAll(),
				'section'	 =>$this->getDefinedTable(Administration\LhakhangSectionTable::class)->getAll(),
				'material'	 =>$this->getDefinedTable(Nangten\MaterialTable::class)->get(array('GroupId'=>12)),
				'category'	 =>$this->getDefinedTable(Nangten\CategoryTable::class)->get(array('group'=>12)),
	...
	{	
		...
		if ($request->isPost()) {
			...
			foreach($pecha as $pechas);
			$dzongkhag	=$this->getDefinedTable(Administration\DistrictTable::class)->getColumn($debris['DzongkhagId'],'DzongkhagNameEn');
			$gewog	=$this->getDefinedTable(Administration\BlockTable::class)->getColumn($debris['GewogId'],'GewogNameEn');
			$lhakhang	=$this->getDefinedTable(Administration\LhakhangTable::class)->getColumn($debris['LhakhangId'],'LhakhangNameEn');
			$folder_name =$this->_dir . '/debri/' . $dzongkhag . '/' .$gewog. '/' .$lhakhang;
			...
			if(in_array($ext, $this->_fileExts)){
				if($file['size'] > $this->_maxSize ){
					...
					if($result){						
						...
						return $this->redirect()->toRoute('debri', array('action' =>'viewpdebri','id'=>$form['debri_id']));					
					}else{
