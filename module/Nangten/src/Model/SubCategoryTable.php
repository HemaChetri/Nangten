...
class SubCategoryTable extends AbstractTableGateway
{
	protected $table = 'ten_subcategory'; //tablename
     ...
     * @return String | Int
     */
    public function getColumn($param, $column, $where = NULL)
    {         
		$where = ( is_array($param) )? $param: array('id' => $param);
		$fetch = array($column);
		$adapter = $this->adapter;       
     ...
     * @return String | Int
     */
	public function getSubcat($column, $where = NULL)
    {         
		//$where = ( is_array($param) )? $param: array('subcat_id' => $param);
		$fetch = array($column);
		$adapter = $this->adapter;       
		$sql = new Sql($adapter);
		$select = $sql->select();
