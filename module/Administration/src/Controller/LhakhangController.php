	...
	*/
	...
	{  
		...
		$id = $this->_id;
			...
			$lhakhangTable = $this->getDefinedTable(Administration\LhakhangTable::class)->getAllReport($data);
	...
	public function lhakhangsectionAction()
	{
		...
		$dzongkhag = $array_id[0];
		$gewog = (sizeof($array_id)>1)?$array_id[1]:'';
		$lhakhang = (sizeof($array_id)>2)?$array_id[2]:'';
		if($this->getRequest()->isPost())
		{
			$form      		 = $this->getRequest()->getPost();
			$dzongkhag  		= $form['dzongkhag'];
	...
	public function getgewogAction()
	{
		...
		$details = $this->getDefinedTable(Administration\BlockTable::class)->get(['DzongkhagId'=>$dzongkhagId]);
			foreach($details as $detail):
				$gewogValue .= "<option value='".$detail['old_id']."'>".$detail['GewogNameDz']."</option>";
			endforeach;	
