...
class LhakhangSectionTable extends AbstractTableGateway
{
	protected $table = 'adm_lhakhang_section'; //tablename
	...
	*/
	...
	public function getReport($data,$where = NULL)
	{
		...
		$select->from($this->table);
		if($data['gewog'] != '-1'){
			$select->where(array('GewogId'=>$data['gewog']));
		}
		if($data['lhakhang'] != '-1'){
			$select->where(array('LhakhangId'=>$data['lhakhang']));
		}
		if($where != NULL):
			$select->where($where);
		endif;		
