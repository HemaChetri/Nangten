			...
			if(in_array($ext, $this->_fileExts)){
				if($file['size'] > $this->_maxSize ){
				   ...
				   if($result){						
						...
						$this->flashMessenger()->addMessage("success^ Stupa Image Uploaded successfully");	
						return $this->redirect()->toRoute('stupa',array('action'=>'viewstupa',''=>$form['stupa_id']));					
					}else{
						$this->flashMessenger()->addMessage("error^ Stupa Image Uploading not successfull");
						return $this->redirect()->toRoute('stupa');
					}
				}
			}
