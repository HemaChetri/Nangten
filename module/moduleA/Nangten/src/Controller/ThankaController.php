			...
			if(in_array($ext, $this->_fileExts)){
				if($file['size'] > $this->_maxSize ){
				   ...
				   if($result){						
						...
						$this->flashMessenger()->addMessage("success^ Thanka Image Uploaded successfully");	
						return $this->redirect()->toRoute('thanka', array('action' =>'viewthanka','id'=>$form['thanka_id']));					
					}else{
						$this->flashMessenger()->addMessage("error^ Thanka Image Uploading not successfull");
						return $this->redirect()->toRoute('thanka');
					}
				}
			}
