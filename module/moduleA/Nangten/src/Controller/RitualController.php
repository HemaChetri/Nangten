			...
			if(in_array($ext, $this->_fileExts)){
				if($file['size'] > $this->_maxSize ){
				   ...
				   if($result){						
						...
						$this->flashMessenger()->addMessage("success^ Ritual Object Image Uploaded successfully");	
						return $this->redirect()->toRoute('ritual', array('action' =>'viewritual','id'=>$form['ritual_id']));					
					}else{
						$this->flashMessenger()->addMessage("error^ Ritual Object Image Uploading not successfull");
						return $this->redirect()->toRoute('ritual');
					}
				}
			}
